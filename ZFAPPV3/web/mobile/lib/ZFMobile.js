...
var ApplicationName = "/ZFN"
var  baseapplicationsrc;
class Sys_Object_Control
{
     constructor(a)
     {
            this.ACTIVE               = a.ACTIVE               ;
            this.ATTRIBUTES1          = a.ATTRIBUTES1          ;
            this.ATTRIBUTES2          = a.ATTRIBUTES2          ;
            this.ATTRIBUTES3          = a.ATTRIBUTES3          ;
            this.CACHEKEY             = a.CACHEKEY             ;
            this.CONTROL_CAPTION      = a.CONTROL_CAPTION      ;
            this.CONTROL_HEIGHT       = a.CONTROL_HEIGHT       ;
            this.CONTROL_LOCATION_X   = a.CONTROL_LOCATION_X   ;
            this.CONTROL_LOCATION_Y   = a.CONTROL_LOCATION_Y   ;
            this.CONTROL_NAME         = a.CONTROL_NAME         ;
            this.CONTROL_WIDTH        = a.CONTROL_WIDTH        ;
            this.CSSCLASS             = a.CSSCLASS             ;
            this.CSS_ELEMENTS         = a.CSS_ELEMENTS         ;
            this.DATABASEVALUE        = a.DATABASEVALUE        ;
            this.DATABASE_KEY         = a.DATABASE_KEY         ;
            this.DATA_LENGTH          = a.DATA_LENGTH          ;
            this.DISABLE              = a.DISABLE              ;
            this.FINDFORMID           = a.FINDFORMID           ;
            this.HTML_ELEMENTS        = a.HTML_ELEMENTS        ;
            this.INPUTVALUE           = a.INPUTVALUE           ;
            this.JAVASCRIPTACTION     = a.JAVASCRIPTACTION     ;
            this.MAX_VALUE            = a.MAX_VALUE            ;
            this.MIN_VALUE            = a.MIN_VALUE            ;
            this.READONLY             = a.READONLY             ;
            this.RELATED_CONTROL_ID   = a.RELATED_CONTROL_ID   ;
            this.REQUIREDFIELDREGULAR = a.REQUIREDFIELDREGULAR ;
            this.REQUIREDFIELDTEXT    = a.REQUIREDFIELDTEXT    ;
            this.SEQUENCE_NUMBER      = a.SEQUENCE_NUMBER      ;
            this.SYS_CONTROL_TYPE_ID  = a.SYS_CONTROL_TYPE_ID  ;
            this.SYS_DATA_TYPE_ID     = a.SYS_DATA_TYPE_ID     ;
            this.SYS_DEFAULT_VALUE_ID  = a.SYS_DEFAULT_VALUE_ID ;
            this.SYS_OBJECT_CONTROL_ID = a.SYS_OBJECT_CONTROL_ID;
            this.TAG                  = a.TAG                  ;
            this.TEXTMODE             = a.TEXTMODE             ;
            this.TOOLTIP              = a.TOOLTIP              ;
            this.VALIDATORCSSCLASS    = a.VALIDATORCSSCLASS    ;

     }
     
}

class Sys_Object_Rule_Control
{
    constructor(c)
    {
            this.SYS_OBJECT_RULE_CONTROL_ID =c.SYS_OBJECT_RULE_CONTROL_ID;    
            this.SYS_OBJECT_ID=             c.SYS_OBJECT_ID;    
            this.RNEW=                      c.RNEW;    
            this.RSAVE=                     c.RSAVE;    
            this.RDELETE=                   c.RDELETE;    
            this.RSEARCH=                   c.RSEARCH;    
            this.RPRINT=                    c.RPRINT ;    
            this.RFIREST=                   c.RFIREST;    
            this.RBACK=                     c.RBACK;    
            this.RNEXT=                     c.RNEXT;    
            this.REND=                      c.REND ;    
            this.REXCELGRID=                c.REXCELGRID;    
            this.REXCELREPORT=              c.REXCELREPORT;    
            this.RDESIGN =                  c.RDESIGN;
    }
}
                         ...
                         if (fullAddress.trim().length > 0 )
							{
									...
									var url =  fullAddress;
								        ...
								        xhr.setRequestHeader("Content-type", "application/jsonn; charset=utf-8");								    
									xhr.send();
                                                    }             			 	
		...
		else
		{
                                    ...
                                    xhr.onreadystatechange = function () {
                                        if (xhr.readyState === 4 && xhr.status === 200) {
                                                ...
                                                try 
                                                {
                                                    outdata = JSON.parse(xhr.responseText);
                                                }
                                                catch (e){}
                                                fn(outdata);
                                        }
...
async function autocompletefx(inp, control_id, Control,hidvalue) {
  ...
  inp.addEventListener("input", async (e) =>  {
    ...
    let arr = await FillFindObjectBoxAndTextSearch(control_id,Control);    
...
async function  GetDataComboDataSource(Formid,Control_Id)
{
     return new Promise(resolve => {getDataComboDataSourceSolver(Formid,Control_Id, resolve);});
}
...
function ZM_SelectRecord(e, row, keyname)
{
     var alldata = row.getData();
     var datavalue = alldata[keyname];
    
    var form_id = GetUrlParameter('ID');
    var ZPPA1 = GetUrlParameter('ZPPA1');
    var ZPPA2 = GetUrlParameter('ZPPA2');
    var ZPPA3 = GetUrlParameter('ZPPA3');
    var navigateaddress = 'zframe.android?ID='+form_id+'&KeyID='+datavalue+'&ZPPA1='+ZPPA1+'&ZPPA2='+ZPPA2+'&ZPPA3='+ZPPA3;
    window.location = navigateaddress;
    
}

function ZM_SelectRecord_WithData(record_id)
{
    ...
    var ZPPA2 = GetUrlParameter('ZPPA2');
    var ZPPA3 = GetUrlParameter('ZPPA3');
    var navigateaddress = 'zframe.android?ID='+form_id+'&KeyID='+record_id+'&ZPPA1='+ZPPA1+'&ZPPA2='+ZPPA2+'&ZPPA3='+ZPPA3;
    window.location = navigateaddress;
...
var FormControl ;
var FormDataSource;
async function ZF_CreateFrom(){
    var main_Container = document.getElementById("control_Container");
    
    var form_id = GetUrlParameter('ID');
    var key_id  = GetUrlParameter('KeyID');
    var ZPPA1   = GetUrlParameter('ZPPA1');
    var ZPPA2   = GetUrlParameter('ZPPA2');
    var ZPPA3   = GetUrlParameter('ZPPA3');
    let   formfilldata = await GetFormDataSource(form_id,key_id);
    let   formstruct =   await GetFormStruct(form_id);        
    
    FormControl   = formstruct;
    ...
    var html = "";
    var a=0 ;
    formstruct = sort_by_key(formstruct,'SEQUENCE_NUMBER');
    for ( a=0 ; a < formstruct.length ; a++  )
    {
        let c = await  Generate_Control_Code(formstruct[a],formfilldata);
            html += c  + "<br>";
    }
...
async function ZF_init(id)
{
    
      var Control_panel = document.getElementById("ControlPanel");
    ...
    */
     ...
     let FromData = await GetFormInformation(form_id , 0)   ;
     let Sys_Object_Rules = await GetFormInformation(form_id , 1)   ;
     let ZFrameFormRuleControls = await GetFormInformation(form_id , 2)   ;
     let JavaScriptData = await GetFormInformation(form_id , 3)   ;
     let TemplateData  = "" ; //await GetFormInformation(form_id , 4)   ;
     var javascriptCode = "";
     var HtmlTemplte = ""; 
     var DESIGN_PATTERN = FromData[0].SYS_DESIGN_PATTERN_ID;
     /*
      * 
      *
            1	GridForm
     ...
     if (JavaScriptData.length > 0)
     {
         var ECODE = JavaScriptData[0].DATACODE;
         javascriptCode =   window.atob(ECODE);
     }
     if (TemplateData.length > 0)
     {
         var TEMEC = TemplateData[0].TEMPLATEDATA;
         HtmlTemplte =   window.atob(TEMEC);
     }
     
     if ( DESIGN_PATTERN == 1)
     {
         // Create Toolsbars 
        // alert("Create Tools Bar");
        var fullControl = true;
        if (ZFrameFormRuleControls.length == 0 )
        {
            fullControl = true;
        }
        else
        {
            ...
            let Ftoolbar = new Sys_Object_Rule_Control(ZFrameFormRuleControls[0]);    
            ...
            if (designmode)
            {
                    CmdCode += '<button type="button" id="cmdback" onclick="goBack()" style="width:25%;" class="btn btn-default">بازگشت </button>';                
            }
            
            if (Ftoolbar.RNEW == 'true')
            {
                
                    CmdCode += '<button type="button" id="cmdnewc" onclick="gotonew()" style="width:25%;" class="btn btn-success">جدید</button>';                
            }
            
            if (designmode)
            {
                if (Ftoolbar.RSAVE == 'true')
                {
                    CmdCode += '<button type="button" id="cmdsave"  onclick="dosave()" style="width:25%;" class="btn btn-primary">ذخیره</button>';
                }

                if (Ftoolbar.RDELETE == 'true')
                {
                     if (key_id != 0 && key_id != null)
                     {
                        CmdCode += '<button type="button" id="cmddelete" onclick="dodelete()" style="width:25%;" class="btn btn-danger">حذف</button>';
                     }
                }
            }   
           
           Control_panel.innerHTML = CmdCode;
           Control_panel.style.direction="rtl";
           fullControl = false;
           ...
           if (designmode)
            ...
            if (designmode)
            {
                ...
                CmdCode += '<button type="button" id="cmdsave" onclick="dosave()" style="width:25%;" class="btn btn-primary">ذخیره</button>';            
                 ...
                 if (key_id != 0 && key_id != null)
                 {
                    CmdCode += '<button type="button" id="cmddelete" onclick="dodelete()" style="width:25%;" class="btn btn-danger">حذف</button><BR>';
                 }
             }
               Control_panel.innerHTML = CmdCode;           
               Control_panel.style.direction="rtl";
...
// From generator region
async function Generate_Control_Code(controldata,datasource)
{
    let nowc = new Sys_Object_Control(controldata);    
    if (nowc.SYS_CONTROL_TYPE_ID == 1)
    {
          var colname  = nowc.CONTROL_NAME;
        var valuec = "" ;
          if (datasource.length > 0)
          {
               valuec= datasource[0][colname];
          }
         return  Create_Heddien(nowc,valuec);
    }
    else if (nowc.SYS_CONTROL_TYPE_ID == 2)
    {
          var colname  = nowc.CONTROL_NAME;
          var valuec = "" ;
          if (datasource.length > 0)
          ...
          return  Create_Text(nowc,valuec);
    }
    else if (nowc.SYS_CONTROL_TYPE_ID == 3)
    {
          var colname  = nowc.CONTROL_NAME;
           var valuec = "" ;
          if (datasource.length > 0)
          ...
          let c = await Create_DataCombo(nowc,valuec);
          return c 
    }
    else if (nowc.SYS_CONTROL_TYPE_ID == 4)
    {
          var colname  = nowc.CONTROL_NAME;
           var valuec = "" ;
          if (datasource.length > 0)
          ...
          return  Create_CheckBox(nowc,valuec);
    }
    else if (nowc.SYS_CONTROL_TYPE_ID == 5)
    {
          var colname  = nowc.CONTROL_NAME;
           var valuec = "" ;
          if (datasource.length > 0)
          ...
          return  Create_Radio(nowc,valuec);
    }       
     else if (nowc.SYS_CONTROL_TYPE_ID == 6)
    {
          var colname  = nowc.CONTROL_NAME;
           var valuec = "" ;
          if (datasource.length > 0)
          ...
          return  Create_DateBox(nowc,valuec);
    } 
     else if (nowc.SYS_CONTROL_TYPE_ID == 7)
    {
          var colname  = nowc.CONTROL_NAME;
           var valuec = "" ;
          if (datasource.length > 0)
          ...
          return  Create_Textaria(nowc,valuec);
    }     
    else if (nowc.SYS_CONTROL_TYPE_ID == 9)
    {
          var colname  = nowc.CONTROL_NAME;
           var valuec = "" ;
          if (datasource.length > 0)
          ...
          return  Create_FindObjectBox(nowc,valuec);
    }
    else if (nowc.SYS_CONTROL_TYPE_ID == 19)
    {
          var colname  = nowc.CONTROL_NAME;
           var valuec = "" ;
          if (datasource.length > 0)
    ...
    /*
        ...
        14	Image	img
        15	CheckBoxList	cbl
        16	RadioButtonList	rbl
        17	Form	frm
        18	DownLoadObject	don
        19	SearchText	stx
        20	Label	lbl
        21	TextEditor	ted
        22	ListBox	lis
...
function Create_Heddien(Sys_Object_Control , value  )
{
    return "<input type=\"hidden\" name=\""+Sys_Object_Control.CONTROL_NAME+"\"  value=\""+value+"\"  ID=\""+Sys_Object_Control.CONTROL_NAME+"\" ZID=\""+Sys_Object_Control.CONTROL_NAME+"\" >"        
}

function Create_Text(Sys_Object_Control, value)
{
    
    var rt = "<label for=\""+Sys_Object_Control.CONTROL_NAME+"\" class=\"col-lg-4\">"+Sys_Object_Control.CONTROL_CAPTION+"</label>";
     rt += "<input class=\"form-control\" dir=\"rtl\" type=\"Text\" name=\""+Sys_Object_Control.CONTROL_NAME+"\" ID=\""+Sys_Object_Control.CONTROL_NAME+"\" ZID=\""+Sys_Object_Control.CONTROL_NAME+"\" value= \""+value+"\" >" ;     
     
     return rt;
    
}

function Create_DateBox(Sys_Object_Control, value)
{
    
    var rt = "<label for=\""+Sys_Object_Control.CONTROL_NAME+"\" class=\"col-lg-4\">"+Sys_Object_Control.CONTROL_CAPTION+"</label>";
     rt += "<input class=\"form-control\" dir=\"rtl\" type=\"text\" name=\""+Sys_Object_Control.CONTROL_NAME+"\" ID=\""+Sys_Object_Control.CONTROL_NAME+"\" ZID=\""+Sys_Object_Control.CONTROL_NAME+"\" value= \""+value+"\" >" ;     
     rt+= "<script> $('."+Sys_Object_Control.CONTROL_NAME+"').persianDatepicker({ " +
           "  observer: true, "+
     "format: 'YYYY/MM/DD', "+ 
     " }); </script>"
     return rt;
...
function Create_CheckBox(Sys_Object_Control, value)
{
    
    var rt = "<div class=\"custom-control custom-checkbox\"><label for=\""+Sys_Object_Control.CONTROL_NAME+"\" class=\"col-lg-4\">"+Sys_Object_Control.CONTROL_CAPTION+"</label>";
    if (value == "true" || value == "1" )
    {
        rt+= "<input type=\"checkbox\"   data-toggle=\"toggle\" id=\""+Sys_Object_Control.CONTROL_NAME+"\" name=\""+Sys_Object_Control.CONTROL_NAME+"\"  checked></div>";
    }
    else
    {
         rt+= "<input type=\"checkbox\"  data-toggle=\"toggle\" id=\""+Sys_Object_Control.CONTROL_NAME+"\" name=\""+Sys_Object_Control.CONTROL_NAME+"\"></div>";
    }
...
function Create_Radio(Sys_Object_Control, value)
{
    var rt = "<div class=\"custom-control custom-radio\"><label for=\""+Sys_Object_Control.CONTROL_NAME+"\" class=\"col-lg-4\">"+Sys_Object_Control.CONTROL_CAPTION+"</label>";
    if (value == "true" || value == "1" )
    {
        rt+= "<input type=\"radio\" data-toggle=\"toggle\" id=\""+Sys_Object_Control.CONTROL_NAME+"\"  name=\""+Sys_Object_Control.CONTROL_NAME+"\" checked></div>";
    }
    else
    {
         rt+= "<input type=\"radio\" data-toggle=\"toggle\"  id=\""+Sys_Object_Control.CONTROL_NAME+"\"  name=\""+Sys_Object_Control.CONTROL_NAME+"\" ></div>";
    }
          
     return rt;
}

function Create_Textaria(Sys_Object_Control, value)
{
    
    var rt = "<div class=\"form-group\" > <label for=\""+ Sys_Object_Control.CONTROL_NAME +"\"> "+Sys_Object_Control.CONTROL_CAPTION+" </label>" ;
        rt +="  <textarea dir=\"rtl\" class=\"form-control\" id=\""+Sys_Object_Control.CONTROL_NAME+"\" rows=\"7\">"+ value +"</textarea></div>";
     return rt;
}


 async function Create_DataCombo(Sys_Object_Control, value)
{
    var rt = "<div class=\"form-group\" > <label for=\""+ Sys_Object_Control.CONTROL_NAME +"\"> "+Sys_Object_Control.CONTROL_CAPTION+" </label>" ;
        rt +=" <div class=\"row\"> <select dir=\"rtl\" id=\""+Sys_Object_Control.CONTROL_NAME+"\" name=\""+Sys_Object_Control.CONTROL_NAME+"\" class=\"form-control\"> ";
   
   
       var form_id = GetUrlParameter('ID');
         try 
                    {
             let  Control_DataSource  = await GetDataComboDataSource( form_id,Sys_Object_Control.SYS_OBJECT_CONTROL_ID);    
                    
                  
                    var a = 0;
                    var keys = Object.keys(Control_DataSource[0]);
                    var Key_Id  = keys[0];
                    var key_value = keys[1];
                       rt+=      " <option  value=\"0\">-</option> " ;
                        for (a=0 ; a < Control_DataSource.length ; a++)
                        {
                           if (Control_DataSource[a][Key_Id] == value)
                           {
                                rt+=      " <option selected=\"selected\" value=\""+Control_DataSource[a][Key_Id] +"\">"+Control_DataSource[a][key_value]+"</option> " ;
                           }
                           else
                           {
                             rt+=      " <option value=\""+Control_DataSource[a][Key_Id] +"\">"+Control_DataSource[a][key_value]+"</option> " ;
                           }       
                          }
                    }catch(e){}
...
async function Create_FindObjectBox(Sys_Object_Control, value)
{
    
    let datatext  =  await GetContenetWithID(Sys_Object_Control.SYS_OBJECT_CONTROL_ID, value)
    if (!datatext)
    {
        datatext = "";    
    }
    ...
    var rt = "<div class=\"form-group\" > <label for=\""+ Sys_Object_Control.CONTROL_NAME +"\"> "+Sys_Object_Control.CONTROL_CAPTION+" </label>" ;
    rt +=" <div class=\"autocomplete\" style=\"width:100%;\"><input type=\"hidden\" id=\""+Sys_Object_Control.CONTROL_NAME+"\"  name=\""+Sys_Object_Control.CONTROL_NAME+"\"  value=\""+value+"\" ><input  value=\""+datatext+"\" dir=\"rtl\" class=\"form-control\"  id=\""+Sys_Object_Control.CONTROL_NAME+"_TXT\" type=\"text\" name=\""+Sys_Object_Control.CONTROL_NAME+"_TXT\" placeholder=\""+Sys_Object_Control.CONTROL_CAPTION+"\"   onfocus=\"autocompletefx("+Sys_Object_Control.CONTROL_NAME+"_TXT,"+Sys_Object_Control.SYS_OBJECT_CONTROL_ID+",this,'"+Sys_Object_Control.CONTROL_NAME+"');\"></div></div>";
...
async function dosave()
{
    ...
    {
        let Now_Control = new Sys_Object_Control(FormControl[i]);    
        {
             var CName =  Now_Control.CONTROL_NAME;
             var Cvalue = '';
             try 
             {
               Cvalue =document.getElementById(CName).value;
             }catch(e){}
           
             param.push({name:CName, value:Cvalue});                          
        }
        
    }
    
    var functionaction='insert';  
    let __resultaction  =  await ZfAction(2,functionaction,param); 
          return __resultaction ; 
